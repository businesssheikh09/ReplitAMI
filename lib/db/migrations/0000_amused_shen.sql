-- Data migration: standardize legacy role values before applying the
-- role check constraint. Must run before the CREATE TABLE statement
-- on an existing DB (or as a pre-migration step on fresh installs).
-- UPDATE users SET role = 'management' WHERE role = 'admin';
-- UPDATE users SET role = 'operations' WHERE role = 'visa';
-- UPDATE users SET role = 'sales'      WHERE role IN ('support');
-- (These are handled automatically by seedDepartmentAccounts() on server boot.)

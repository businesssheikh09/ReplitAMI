...
var doc = new import_pdfkit.default({ size: "A4", margin: 0, autoFirstPage: false });
var outPath = import_path.default.join(process.cwd(), "project-map.pdf");
var writeStream = import_fs.default.createWriteStream(outPath);
...
function footer(left) {
  ...
  doc.fontSize(7.5).font("Helvetica").fillColor("#94a3b8").text(left, M, H - 17, { width: CW / 2, align: "left" });
  doc.fillColor("#94a3b8").text(
    `Page ${pageNum}  \u2022  Al Musafir International ERP \u2014 Project Map  \u2022  ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}`,
    M,
    H - 17,
...
function sectionBar(label, x, y, w, color) {
  ...
  doc.fontSize(9).font("Helvetica-Bold").fillColor(C.white).text(label, x + 6, y + 6, { width: w - 12, lineBreak: false });
  return y + 24;
}
function drawBox(x, y, w, h, fill, stroke, r = 6) {
  doc.roundedRect(x, y, w, h, r);
  if (stroke) doc.fillAndStroke(fill, stroke);
  else doc.fill(fill);
}
function arrowH(x1, y, x2, color) {
  const aLen = 7;
  doc.save();
  doc.moveTo(x1, y).lineTo(x2 - aLen, y).strokeColor(color).lineWidth(1.5).stroke();
  doc.moveTo(x2, y).lineTo(x2 - aLen, y - 4).lineTo(x2 - aLen, y + 4).closePath().fill(color);
  doc.restore();
}
function arrowHL(x1, y, x2, color) {
  const aLen = 7;
  doc.save();
  doc.moveTo(x1, y).lineTo(x2 + aLen, y).strokeColor(color).lineWidth(1.5).stroke();
  doc.moveTo(x2, y).lineTo(x2 + aLen, y - 4).lineTo(x2 + aLen, y + 4).closePath().fill(color);
...
function arrowV(x, y1, y2, color) {
  const aLen = 7;
  doc.save();
  doc.moveTo(x, y1).lineTo(x, y2 - aLen).strokeColor(color).lineWidth(1.5).stroke();
  doc.moveTo(x, y2).lineTo(x - 4, y2 - aLen).lineTo(x + 4, y2 - aLen).closePath().fill(color);
  doc.restore();
}
function table(x, y, totalW, headers, rows, colW, headerBg, rowH = 20, fontSize = 8) {
  doc.rect(x, y, totalW, 22).fill(headerBg);
  let cx = x;
  headers.forEach((h, i) => {
    doc.fontSize(8.5).font("Helvetica-Bold").fillColor(C.white).text(h, cx + 4, y + 7, { width: colW[i] - 8, lineBreak: false, align: "left" });
    cx += colW[i];
  });
  let ry = y + 22;
  rows.forEach((row, ri) => {
    ...
    row.forEach((cell, ci) => {
      ...
      if (typeof raw === "string") {
        ...
        const seg = raw;
        doc.fontSize(fontSize).font(seg.bold ? "Helvetica-Bold" : "Helvetica").fillColor(seg.color ?? C.text).text(seg.text, tx, ry + (rowH - fontSize - 2) / 2, { width: cellW, lineBreak: false });
      }
      cx2 += colW[ci];
    });
...
var logoX = M;
var logoY = 60;
drawBox(logoX, logoY, 70, 70, C.sky, void 0, 8);
doc.fontSize(10).font("Helvetica-Bold").fillColor(C.white).text("AMI", logoX, logoY + 28, { width: 70, align: "center" });
doc.fontSize(28).font("Helvetica-Bold").fillColor(C.white).text("Al Musafir International", M + 85, logoY + 8, { width: CW - 85 });
doc.fontSize(13).font("Helvetica").fillColor("#93c5fd").text("Umrah Travel ERP \u2014 Complete Project Map", M + 85, logoY + 44, { width: CW - 85 });
doc.rect(M, logoY + 80, CW, 1.5).fill(C.sky);
var descY = logoY + 100;
doc.fontSize(11).font("Helvetica").fillColor("#e2e8f0").text(
  "This document is a complete reference guide for the Al Musafir International Umrah travel ERP. It maps every module, every user role, and how data flows between the staff dashboard, public website, and API server. Use it to understand the system at a glance, onboard new developers, or review the full development cost estimate.",
  M,
  descY,
  { width: CW, lineGap: 3 }
);
...
doc.fontSize(11).font("Helvetica-Bold").fillColor(C.sky).text("WHAT'S INSIDE", M + 16, boxY + 16, { width: CW - 32 });
var contents = [
...
contents.forEach(([num, text]) => {
  doc.fontSize(9).font("Helvetica-Bold").fillColor(C.sky).text(num + ".", M + 16, cy, { width: 18, lineBreak: false });
  doc.fontSize(9).font("Helvetica").fillColor("#cbd5e1").text(text, M + 34, cy, { width: CW - 50, lineBreak: false });
  cy += 17;
});
var chipY = boxY + 230;
doc.fontSize(9).font("Helvetica-Bold").fillColor("#94a3b8").text("BUILT WITH:", M, chipY);
var chips = ["Node.js 24", "TypeScript", "React + Vite", "Express 5", "PostgreSQL", "Drizzle ORM", "pnpm Workspaces"];
var chipX = M + 68;
chips.forEach((chip) => {
  const cw2 = doc.widthOfString(chip) + 14;
  drawBox(chipX, chipY - 2, cw2, 16, "#2d4a6b", "#334155", 3);
  doc.fontSize(7.5).font("Helvetica").fillColor("#94a3b8").text(chip, chipX + 7, chipY + 2, { lineBreak: false });
  chipX += cw2 + 6;
});
doc.fontSize(8.5).font("Helvetica").fillColor("#64748b").text(
  `Generated: ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" })}   \u2022   Confidential \u2014 Internal Use Only`,
  M,
  H - 50,
...
doc.fontSize(7.5).font("Helvetica-Bold").fillColor(C.navy).text("Staff Only  \u2022  Requires Login", p2erpX + 6, p2row1Y + 32, { width: p2bW - 12, lineBreak: false });
["Manage clients & quotations", "Hotel / Flight / Transport / Visa", "Accounting & vouchers", "WhatsApp automation", "4 roles: Management, Sales,", "  Accounts, Operations"].forEach((b, i) => {
  doc.fontSize(7).font("Helvetica").fillColor(C.text).text("\u2022 " + b, p2erpX + 6, p2row1Y + 46 + i * 10, { width: p2bW - 12, lineBreak: false });
});
doc.fontSize(7).font("Helvetica-Bold").fillColor(C.muted).text("/ (ERP root path)", p2erpX + 6, p2row1Y + p2bH - 14, { width: p2bW - 12, lineBreak: false });
drawBox(p2apiX, p2row1Y, p2bW, p2bH, "#faf5ff", C.purple, 8);
doc.rect(p2apiX, p2row1Y, p2bW, 26).fill(C.purple);
doc.fontSize(10).font("Helvetica-Bold").fillColor(C.white).text("API Server", p2apiX + 6, p2row1Y + 8, { width: p2bW - 12, lineBreak: false });
doc.fontSize(7.5).font("Helvetica-Bold").fillColor(C.purple).text("Central Backend  \u2022  Express 5", p2apiX + 6, p2row1Y + 32, { width: p2bW - 12, lineBreak: false });
["REST endpoints for all features", "JWT authentication middleware", "Drizzle ORM \u2192 PostgreSQL", "OpenAPI contract-first design", "20+ route handler files", "esbuild compiled bundle"].forEach((b, i) => {
  doc.fontSize(7).font("Helvetica").fillColor(C.text).text("\u2022 " + b, p2apiX + 6, p2row1Y + 46 + i * 10, { width: p2bW - 12, lineBreak: false });
});
doc.fontSize(7).font("Helvetica-Bold").fillColor(C.muted).text("/api  \u2022  Port 5000", p2apiX + 6, p2row1Y + p2bH - 14, { width: p2bW - 12, lineBreak: false });
drawBox(p2webX, p2row1Y, p2bW, p2bH, "#f0f9ff", C.sky, 8);
doc.rect(p2webX, p2row1Y, p2bW, 26).fill(C.sky);
doc.fontSize(10).font("Helvetica-Bold").fillColor(C.white).text("Public Website", p2webX + 6, p2row1Y + 8, { width: p2bW - 12, lineBreak: false });
doc.fontSize(7.5).font("Helvetica-Bold").fillColor(C.sky).text("Customers  \u2022  Public + Portal", p2webX + 6, p2row1Y + 32, { width: p2bW - 12, lineBreak: false });
["Browse packages & flights", "Flight search & GDS booking", "Package inquiry form", "Customer portal login", "View invoices / vouchers / visa", "Download documents"].forEach((b, i) => {
  doc.fontSize(7).font("Helvetica").fillColor(C.text).text("\u2022 " + b, p2webX + 6, p2row1Y + 46 + i * 10, { width: p2bW - 12, lineBreak: false });
});
doc.fontSize(7).font("Helvetica-Bold").fillColor(C.muted).text("/frontend  \u2022  Public", p2webX + 6, p2row1Y + p2bH - 14, { width: p2bW - 12, lineBreak: false });
var p2mid = p2row1Y + p2bH / 2;
arrowH(p2erpX + p2bW, p2mid, p2apiX, C.navy);
arrowHL(p2webX, p2mid, p2apiX + p2bW, C.sky);
doc.fontSize(7).font("Helvetica").fillColor(C.navy).text("REST", p2erpX + p2bW + 2, p2mid - 8, { width: p2gap - 4, lineBreak: false });
doc.fontSize(7).font("Helvetica").fillColor(C.sky).text("REST", p2webX - p2gap + 2, p2mid - 8, { width: p2gap - 4, lineBreak: false });
var p2dbY = p2row1Y + p2bH + 55;
var p2dbW = p2bW;
var p2dbX = p2apiX;
drawBox(p2dbX, p2dbY, p2dbW, 72, "#f0fdf4", C.green, 8);
doc.rect(p2dbX, p2dbY, p2dbW, 26).fill(C.green);
doc.fontSize(10).font("Helvetica-Bold").fillColor(C.white).text("PostgreSQL Database", p2dbX + 6, p2dbY + 8, { width: p2dbW - 12, lineBreak: false });
["All persistent data storage", "Drizzle ORM + type-safe queries", "30+ tables across all domains"].forEach((b, i) => {
  doc.fontSize(7).font("Helvetica").fillColor(C.text).text("\u2022 " + b, p2dbX + 6, p2dbY + 31 + i * 10, { width: p2dbW - 12, lineBreak: false });
});
arrowV(p2apiX + p2bW / 2, p2row1Y + p2bH, p2dbY, C.purple);
doc.fontSize(7).font("Helvetica").fillColor(C.purple).text("SQL", p2apiX + p2bW / 2 + 4, p2row1Y + p2bH + 18, { width: 40, lineBreak: false });
var p2panelY = p2dbY + 80;
var p2panelW = Math.floor(CW / 3) - 6;
var p2panels = [
...
p2panels.forEach((p, i) => {
  ...
  doc.fontSize(8.5).font("Helvetica-Bold").fillColor(p.color).text(p.label, px + 8, p2panelY + 8, { width: p2panelW - 16, lineBreak: false });
  p.items.forEach((item, j) => {
    doc.fontSize(7.5).font("Helvetica").fillColor(C.text).text("\u2022 " + item, px + 8, p2panelY + 26 + j * 14, { width: p2panelW - 16, lineBreak: false });
  });
});
...
secFeatures.forEach(([title, desc]) => {
  ...
  doc.fontSize(8.5).font("Helvetica-Bold").fillColor(C.navy).text(title, M + 8, y3 + 6, { width: 150, lineBreak: false });
  doc.fontSize(8).font("Helvetica").fillColor(C.text).text(desc, M + 165, y3 + 7, { width: CW - 175, lineBreak: false });
  y3 += 44;
});
...
groups.forEach((g) => {
  ...
  g.items.forEach(([name, path2, desc]) => {
    doc.fontSize(8).font("Helvetica-Bold").fillColor(C.text).text(name, M + 6, y4, { width: 120, lineBreak: false });
    doc.fontSize(7.5).font("Helvetica").fillColor(C.muted).text(path2, M + 130, y4, { width: 120, lineBreak: false });
    doc.fontSize(7.5).font("Helvetica").fillColor(C.text).text(desc, M + 258, y4, { width: CW - 262, lineBreak: false });
    y4 += 12;
  });
  y4 += 6;
});
...
var steps5 = [
  ...
  {
    num: "2",
    title: "Browses Packages",
    where: "Public Website  \u2022  /packages/:id",
    color: C.sky,
    desc: "Customer views individual Umrah packages with details, inclusions, pricing, and hotel information."
  },
  {
    num: "3",
    title: "Submits an Inquiry or Books a Flight",
    where: "Public Website  \u2022  /flights  or  Package Inquiry Form",
    color: C.teal,
  ...
  {
    num: "4",
    title: "Staff Receives & Responds",
    where: "ERP  \u2022  /crm  or  /quotations",
    color: C.navy,
  ...
  {
    num: "5",
    title: "Customer Registers on Portal",
    where: "Public Website  \u2022  /portal-register",
    color: C.violet,
    desc: "After confirming interest, the customer registers for the self-service portal. Account starts as 'Pending Approval'."
  },
  {
    num: "6",
    title: "Staff Approves Portal Account",
    where: "ERP  \u2022  /portal-users",
    color: C.amber,
  ...
  {
    num: "7",
    title: "Customer Logs into Portal",
    where: "Public Website  \u2022  /portal-login",
    color: C.violet,
    desc: "Customer logs in with their email/phone and password. They land on their personal dashboard showing booking status."
  },
  {
    num: "8",
    title: "Views Booking Documents",
    where: "Portal  \u2022  /portal/invoices  \u2022  /portal/hotel-vouchers  \u2022  /portal/visa",
    color: C.green,
...
steps5.forEach((s, i) => {
  ...
  doc.fontSize(9).font("Helvetica-Bold").fillColor(C.white).text(s.num, dotX - 9, dotY - 5, { width: 18, align: "center", lineBreak: false });
  if (i < steps5.length - 1) {
    doc.moveTo(dotX, dotY + 9).lineTo(dotX, y5 + stepH - 2).strokeColor("#cbd5e1").lineWidth(1.5).stroke();
  }
  ...
  doc.fontSize(9.5).font("Helvetica-Bold").fillColor(s.color).text(s.title, bx + 44, y5 + 6, { width: CW - 54, lineBreak: false });
  doc.fontSize(7.5).font("Helvetica-Bold").fillColor(C.muted).text(s.where, bx + 44, y5 + 19, { width: CW - 54, lineBreak: false });
  doc.fontSize(8).font("Helvetica").fillColor(C.text).text(s.desc, bx + 44, y5 + 31, { width: CW - 54, lineBreak: false });
  y5 += stepH;
});
...
dbGroups.forEach((g) => {
  ...
  g.tables.forEach(([tname, desc, links]) => {
    doc.fontSize(8).font("Helvetica-Bold").fillColor(g.color).text(tname, M + 6, y7, { width: 130, lineBreak: false });
    doc.fontSize(7.5).font("Helvetica").fillColor(C.text).text(desc, M + 140, y7, { width: 170, lineBreak: false });
    doc.fontSize(7.5).font("Helvetica").fillColor(C.muted).text(links, M + 315, y7, { width: CW - 319, lineBreak: false });
    y7 += 12;
  });
  y7 += 5;
});
...
var workflows = [
  ...
  },
  {
    ...
    steps: [
      ...
      "System auto-multiplies: Rate \xD7 Nights \xD7 Rooms = Receivable Total.",
...
workflows.forEach((wf, wi) => {
  ...
  drawBox(xBase, yRef, colW, 20, wf.color, void 0, 4);
  doc.fontSize(8.5).font("Helvetica-Bold").fillColor(C.white).text(wf.title, xBase + 6, yRef + 6, { width: colW - 12, lineBreak: false });
  let curY = yRef + 24;
  wf.steps.forEach((step, si) => {
    const numW = 14;
    doc.roundedRect(xBase + 6, curY, numW, 12, 2).fill(wf.color);
    doc.fontSize(7).font("Helvetica-Bold").fillColor(C.white).text(String(si + 1), xBase + 6, curY + 2, { width: numW, align: "center", lineBreak: false });
    doc.fontSize(7.5).font("Helvetica").fillColor(C.text).text(step, xBase + numW + 10, curY, { width: colW - numW - 20, lineBreak: false });
    curY += 14;
  });
  curY += 8;
  if (isRight) y8R = curY;
...
folders.forEach((f, i) => {
  ...
  doc.fontSize(8).font("Helvetica-Bold").fillColor(f.color).text(f.path, fx + 6, fy + 4, { width: CW / 2 - 20, lineBreak: false });
  doc.fontSize(7.5).font("Helvetica").fillColor(C.text).text(f.desc, fx + 6, fy + 13, { width: CW / 2 - 20, lineBreak: false });
});
footer("Developer Quick-Reference");
function computeAgentSessions() {
  let raw = "";
  try {
    raw = (0, import_child_process.execSync)('git log --pretty=format:"%ai %s"', { encoding: "utf8", timeout: 1e4 });
  } catch {
    return [];
  }
  const commits = [];
  for (const line of raw.trim().split("\n")) {
    ...
    if (m) {
      ...
      if (!isNaN(ts)) commits.push({ ts, msg: m[2] });
    }
  }
  commits.sort((a, b) => a.ts - b.ts);
  const GAP = 2 * 60 * 60 * 1e3;
  const clusters = [];
  let cur = [];
  for (let i = 0; i < commits.length; i++) {
    if (i === 0 || commits[i].ts - commits[i - 1].ts > GAP) {
  ...
  return clusters.map((cluster) => {
    ...
    const minutes = Math.round((last.ts - first.ts) / 6e4);
    const date = new Date(first.ts).toISOString().slice(0, 10);
    const best = cluster.filter((c) => !c.msg.startsWith("Transitioned") && !c.msg.startsWith("Initial") && !c.msg.startsWith("Saved progress")).sort((a, b) => b.msg.length - a.msg.length)[0] ?? cluster[cluster.length - 1];
    const raw2 = best.msg.replace(/^(Task #\d+:|feat:|fix:|chore:|Task \d+)/i, "").trim();
    const label = raw2.length > 72 ? raw2.slice(0, 69) + "..." : raw2;
    return { date, label: label || best.msg, minutes };
  }).filter((s) => s.minutes > 0);
}
var agentSessions = computeAgentSessions();
var totalAgentMinutes = agentSessions.reduce((s, r) => s + r.minutes, 0);
var totalAgentHours = totalAgentMinutes / 60;
var RATE = 5;
var PKR_RATE = 290;
newPage();
doc.rect(0, 0, W, 70).fill(C.gold);
doc.rect(0, 0, W, 6).fill("#92400e");
doc.fontSize(17).font("Helvetica-Bold").fillColor(C.white).text("Actual Agent Working Time", M, 16, { width: CW });
doc.fontSize(9).font("Helvetica").fillColor("#f1f5f9").text(
  `Live from git history  \u2022  $${RATE}/hr agent rate  \u2022  1 USD = ${PKR_RATE} PKR  \u2022  Generated ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}`,
  M,
  40,
  { width: CW }
);
var cw10 = [70, 228, 55, 82, 80];
var totalCW10 = cw10.reduce((s, v) => s + v, 0);
var y10 = 80;
doc.rect(M, y10, totalCW10, 22).fill(C.gold);
var hdrs10 = ["Date", "Primary Work / Session", "Time", "Cost (USD)", "Cost (PKR)"];
var hx = M;
hdrs10.forEach((h, i) => {
  doc.fontSize(8).font("Helvetica-Bold").fillColor(C.white).text(h, hx + 4, y10 + 7, { width: cw10[i] - 8, lineBreak: false, align: i >= 2 ? "right" : "left" });
  hx += cw10[i];
});
y10 += 22;
agentSessions.forEach((row, ri) => {
  const rh = 17;
  doc.rect(M, y10, totalCW10, rh).fill(ri % 2 === 0 ? C.white : "#fef9ec");
  const hrs = row.minutes / 60;
  const usd = hrs * RATE;
  const pkr = usd * PKR_RATE;
  const hh = Math.floor(row.minutes / 60);
  const mm = row.minutes % 60;
  const dur = hh > 0 ? `${hh}h ${mm}m` : `${mm}m`;
  const cells = [row.date, row.label, dur, `$${usd.toFixed(2)}`, `Rs ${Math.round(pkr).toLocaleString()}`];
  let rx = M;
  cells.forEach((cell, ci) => {
    doc.fontSize(ci === 1 ? 6.8 : 7.5).font(ci === 0 ? "Helvetica-Bold" : "Helvetica").fillColor(ci === 0 ? C.navy : C.text).text(cell, rx + 4, y10 + (rh - 8) / 2, {
      width: cw10[ci] - 8,
      lineBreak: false,
  ...
  doc.rect(M, y10, totalCW10, rh).lineWidth(0.25).stroke("#f0e6c8");
  y10 += rh;
});
doc.rect(M, 80, totalCW10, y10 - 80).lineWidth(0.75).stroke(C.gold);
y10 += 6;
drawBox(M, y10, totalCW10, 56, "#fffbeb", C.gold, 5);
var totalUSD = totalAgentHours * RATE;
var totalPKR = totalUSD * PKR_RATE;
var totalHDisplay = totalAgentHours % 1 === 0 ? `${totalAgentHours}` : totalAgentHours.toFixed(1);
doc.circle(M + 10, y10 + 13, 4).fill(C.green);
doc.fontSize(8.5).font("Helvetica-Bold").fillColor(C.text).text("TOTAL ACTIVE AGENT TIME:", M + 18, y10 + 8, { lineBreak: false });
doc.fontSize(8.5).font("Helvetica-Bold").fillColor(C.green).text(
  `${totalAgentMinutes} min  (${totalHDisplay} hrs)`,
  M + 18,
  y10 + 8,
  { width: totalCW10 - 26, align: "right", lineBreak: false }
);
doc.rect(M + 8, y10 + 27, totalCW10 - 16, 0.5).fill("#d4b896");
doc.fontSize(9.5).font("Helvetica-Bold").fillColor(C.text).text(`TOTAL COST  (${totalHDisplay} hrs @ $${RATE}/hr):`, M + 8, y10 + 33, { lineBreak: false });
doc.fontSize(10).font("Helvetica-Bold").fillColor(C.green).text(`USD $${totalUSD.toFixed(2)}`, M + 8, y10 + 32, { width: totalCW10 - 110, align: "right", lineBreak: false });
doc.fontSize(10).font("Helvetica-Bold").fillColor(C.amber).text(`PKR ${Math.round(totalPKR).toLocaleString()}`, M + 8, y10 + 32, { width: totalCW10 - 16, align: "right", lineBreak: false });
y10 += 64;
doc.fontSize(7).font("Helvetica").fillColor(C.muted).text(
  "NOTE: Times are computed from git commit timestamps \u2014 each session spans from the first commit to the last commit before a gap of more than 2 hours. Single-commit sessions (0 min) are excluded. Market rate for an equivalent mid-senior developer team would be $35\u201350/hr. Does not include third-party service costs (hosting, WhatsApp Business API, SMS, GDS/Amadeus fees).",
  M,
  y10,
  { width: totalCW10, lineGap: 2 }
);
footer("Actual Agent Working Time");
writeStream.on("finish", () => {
  console.log(`
PDF generated: ${outPath}`);
  console.log(`Pages: ${pageNum}  |  Agent sessions: ${agentSessions.length}  |  Total time: ${totalAgentMinutes} min (${totalHDisplay}h)  |  Cost @ $${RATE}/hr: $${totalUSD.toFixed(2)}`);
  process.exit(0);
});
